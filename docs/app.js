...
var ListView = function ListView() {
  return {
    view: function view(_ref2) {
      ...
      return m("ul.frow-container", invites.map(function (invite) {
        return m("li.shadow-light", [invite.title, invite.startTime]);
      }));
    }
  };
};

var Day = function Day(_ref3) {
...
var Event = function Event(_ref) {
  ...
  return {
    oninit: load,
    view: function view() {
      return m(".event-page", [state.status == "loading" && m(".", "Fetching Event..."), state.status == "failed" && m(".code", state.error.message), state.status == "success" && m(".event-container", [m("h1", data.event.title), m("h3", "".concat(data.event.date, ": ").concat(data.event.startTime, " - ").concat(data.event.endTime)), m(".accordian", [m(".accordian-item.full-width", [m(".accordian-item-title", [m(".frow columns", m(".col-xs-1-2", m("h4", "INFO")), m(".frow row-end col-xs-1-3", m("button", {
        onclick: function onclick(e) {
          return state["info"].show(!state["info"].show());
        }
      }, "<")))]), state.info.show() && m(".accordian-item-body.column", [m("label", "notes", m("p", data.event.notes)), m(".map", {
        style: {
          width: "500px",
          height: "500px"
        },
        oncreate: setupMap
      })])]), m(".accordian-item.full-width", [m(".accordian-item-title", [m(".frow columns", m(".col-xs-1-2", m("h4", "RSVP")), m(".frow row-end col-xs-1-3", m("button", {
        onclick: function onclick(e) {
          return state["rsvp"].show(!state["rsvp"].show());
        }
      }, "<")))]), state.rsvp.show() && m(".accordian-item-body.column", [m("label", m("select", {
        oninput: function oninput(e) {
      ...
      })), "Status: ")])]), m(".accordian-item.full-width", [m(".accordian-item-title", [m(".frow columns", m(".col-xs-1-2", m("h4", "EDIT EVENT")), m(".frow row-end col-xs-1-3", m("button", {
        onclick: function onclick(e) {
          return state["edit"].show(!state["edit"].show());
        }
      }, "<")))]), state.edit.show() && m(".accordian-item-body.column", [m("button", {
        onclick: function onclick(e) {
