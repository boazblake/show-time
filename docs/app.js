...
var Hamburger = function Hamburger() {
  return {
    view: function view(_ref2) {
      ...
      }, mdl.Sidebar.isShowing() ? m(_cjs.CloseLine) : [mdl.State.notifications().any() && [m(".notif-count", mdl.State.notifications().length), m(_cjs.BellLine)], m(_cjs.BarsLine)])];
    }
  };
};
...
var Sidebar = function Sidebar() {
  ...
  var toItemViewModel = function toItemViewModel(items) {
    return Object.entries((0, _ramda.reduceBy)(function (acc, _ref) {
  ...
  var load = function load(_ref3) {
    ...
    var onError = function onError(error) {
      state.load.error((0, _Utils.jsonCopy)(error));
      state.load.status = "failed";
      console.log("error", error);
    };

    var onSuccess = function onSuccess(items) {
  ...
  return {
    ...
    view: function view(_ref4) {
      ...
      return m(".sidebar-page", [m(".sidebar-tab-section", m(".frow row ", [m("button.sidebar-tab.col-xs-1-3", {
        ...
        onclick: function onclick(e) {
          return showState(e.target.innerHTML);
        }
      }, "Home"), m("button.sidebar-tab.col-xs-1-3", {
        ...
        class: "col-xs-1-3"
      }, "Logout"))])), state.Home.isShowing() && m(".sidebar-section", [m(".frow column-center", [m(".sidebar-article", [m("p.sidebar-section-heading", "Items"), m(".ul", state.Home.data.items.map(function (_ref5) {
