...
var Editor = function Editor(_ref) {
  ...
  var updateEvent = function updateEvent(_ref3) {
    ...
    state.isSubmitted = true;
    (0, _validations.validateTask)(data).chain(function () {
  ...
  return {
    view: function view(_ref2) {
      ...
      return m(".comments-items", m(".frow row-start", [m(".frow width-100", [m(".event-comment-textbox-container", m(".frow items-end", [m(".col-xs-4-5", m("textarea.comments-message-container", {
        ...
        return m(".frow column-center width-100 mb-40", {
          oncreate: function oncreate(_ref4) {
            var dom = _ref4.dom;
            l < data.comments.length && (0, _Styles.Animate)(_Styles.slideInRight)({
              dom: dom
            });
          },
          onbeforeremove: function onbeforeremove(_ref5) {
            var dom = _ref5.dom;
            l--;
            return (0, _Styles.Animate)(_Styles.slideOutLeft)({
              dom: dom
            });
          }
        }, m(".event-comments-message-container ".concat(mdl.User.objectId == comment.guestId ? "me" : "other"), m(".event-comments-message", [m(".speech-bubble", [m(".frow.text-left.pt-10", comment.message), mdl.User.objectId == comment.guestId && m(_cjs.TimesCircleLine, {
          onclick: function onclick(e) {
  ...
  return {
    view: function view(_ref2) {
      var _ref2$attrs = _ref2.attrs,
          ...
          data = _ref2$attrs.data,
          state = _ref2$attrs.state;
      return m(".event-guests", m(".guests-container", [m(".event-forms", m(".frow row event-input-group", [m("input.col-xs-4-5 pb-20", {
        placeholder: "email",
        type: "email",
  ...
  return {
    view: function view(_ref2) {
      ...
      return m(".event-info", m(".frow column-start height-100 justify-evenly", [m("h3.heading", "Hosted by: ".concat(data.event.hostId.name)), m(".events-map-container", {
        ...
        oncreate: setupMap
      }), m("label", "".concat(data.event.location)), m("h3.heading", "Guests: Invited ".concat(invitedGuests(data.guests), " , Accepted  ").concat(acceptedGuests(data.guests), " , Total: ").concat(data.guests.length)), m("h3.heading", "Items: Total ".concat(data.items.length, ", selected: ").concat(itemsSelected(data.guests))), m("h3.heading", "Notes: ".concat(data.event.notes)), m("button.required-field", {
        onclick: function onclick(e) {
  ...
  return {
    ...
    view: function view() {
      ...
      return m(".event-page", [state.status == "loading" && m(".", "Fetching Event..."), state.status == "failed" && m(".code", state.error.message), state.status == "success" && m(".width-100", [m(".event-page-heading width-100", [state.modal.isShowing() == "isLast" && m(_Components.Modal, {
        ...
        header: m("h2.frow.text-center", "Your The Last to Leave! Click OK to Delete This Event!"),
        ...
        body: m("ul.frow direction-column", otherGuests(data.guests).filter((0, _ramda.propEq)("status", 1)).map(function (_ref6) {
          var name = _ref6.name,
