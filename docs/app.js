  ...
  return {
    ...
    view: function view() {
      return m(".event-page", [state.status == "loading" && m(".", "Fetching Event..."), state.status == "failed" && m(".code", state.error.message), state.status == "success" && m(".centered-column", [m("h1.", data.event.title), m("h3", "".concat(data.event.date, " | ").concat(data.event.startTime, " - ").concat(data.event.endTime)), m(".accordian", [m(_Components.AccordianItem, {
        ...
        part: "comments",
        title: "Comments"
      }, m(".frow row-start", [m(".frow width-100", [m(".events-messages-container", data.comments.any() ? data.comments.map(function (comment) {
        return m(".frow column-center width-100", m(".event-comments-message-container ".concat(mdl.User.objectId == comment.userId ? "me" : "other"), m(".event-comments-message frow items-end", [m(".speech-bubble", comment.message), m(".comment-name", comment.name)])));
      }) : m("", "Start a conversation")), m(".frow items-end", [m(".col-xs-4-5", m("textarea.comments-message-container", {
        row: 20,
        cols: 50,
        placeholder: "Say hi...",
