...
var updateShowDetails = mdl => update => (0, _fns.updateShowDetailsTask)(mdl)(_Http.default)(update).chain(_ => (0, _fns.getShowDetailsTask)(_Http.default)(mdl.state.details.selected().objectId)).fork(dto => {
  onError(dto);
  (0, _fns.makeToast)({
    mdl,
    status: false,
...
var Episode = () => {
  ...
  var getEpisode = mdl => episode => (0, _fns.getEpisodeTask)(_Http.default)(episode).fork(onError, dto => data = dto);
...
var Home = () => {
  return {
    ...
    view: (_ref2) => {
      ...
      return mdl.state.details.selected() ? m(_components.Modal, {
      ...
      }, (0, _fns.filterShowsByListType)(mdl).map(show => show.listStatus == mdl.state.currentList() && m('ion-item-sliding', m('ion-item', {
        onclick: () => {
          showModal(mdl, show);
          mdl.state.listDom.closeSlidingItems();
        }
        ...
        src: show.image
      })), m('ion-label', m('h2', show.name), m('p', m('i', show.status)), m('p', show.notes))), m('ion-item-options', {
        side: 'start'
      }, m('ion-item-option', {
        onclick: () => {
